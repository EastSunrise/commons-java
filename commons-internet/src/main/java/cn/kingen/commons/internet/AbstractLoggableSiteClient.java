...
/**
 * This class provides a skeletal implementation of a loggable site whose cookies that containing logon information will
 * be stored locally as a browser does.
 *
 * @author Kingen
 */
@Slf4j
    ...
    protected static HttpClientContext loadContext(HttpHost host) {
        ...
        if (file.canRead()) {
            try (ObjectInputStream stream = new ObjectInputStream(FileUtils.openInputStream(file))) {
                ...
                log.warn("Failed to load context of {}", host);
            }
        }
        return context;
    }
